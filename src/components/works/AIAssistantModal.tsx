...
import { getPromptsByType, getArchivesByWorkId } from '@/data';
...
export const AIAssistantModal: React.FC<AIAssistantModalProps> = ({
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      systemPromptContent += `<提示词内容>${promptContent}</提示词内容>`;
      ...
      // 记录原始提示词（仅用于调试）
      console.log('原始提示词内容:', promptContent);
        ...
        <div className="flex items-center">
          {usage && (
            <div className="text-sm text-text-medium bg-gray-50 px-3 py-1.5 rounded-lg border">
              ...
              已使用 {usage.total_tokens} tokens (输入: {usage.prompt_tokens}, 输出: {usage.completion_tokens})
            </div>
          )}
        </div>
